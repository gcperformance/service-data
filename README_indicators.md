...
### `si.csv`
*snapshots available: 2025-03-01, 2026-01-19*
...
A handful of services are excluded from consideration due to being duplicative, outliers, inconsistently reported, or for alignment with previous years reporting:
...
|`1242`| Canadian Passport |ESDC|
|`3602`| Canadian Dental Care Plan Eligibility Verification and Information|ESDC|
...
- `qa_no_volume_variation`: Boolean value (True or False) indicating whether the reported total application volume is the same every year for at least 4 years, including the latest reporting year. Excludes services with 0 application volume.
- `qa_extreme_volume_variation`: Boolean value (True or False) indicating whether total application volume reported exhibits an abnormally large variation in the latest reporting year compared to at least three previous years. This assumes that application volume over time follows a normal distribution, where values tend to be stable around the mean. A value or `True` is returned when the latest fiscal year's application volume differs from the mean of the preceding years by more than 20 standard deviations (see variable `stdevs_away_from_mean_threshold`); otherwise the value is false.
- `fy_num_applications_total`: Concatenated result of all fiscal years and total application amounts for the service. Provides context for the qa issues relating to application volume variance.
