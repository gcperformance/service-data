...
- `qa_no_volume_variation`: Boolean value (True or False) indicating whether total application volume was the same for at least 4 years with non-zero values, including the latest reporting year.
- `qa_extreme_volume_variation`: Boolean value (True or False) indicating whether total application volume reported an unexpectedly large variation in the latest reporting year compared to 3 or more other previous years with non-zero values. This assumes that application volume over time follows a normal distribution, where values tend to be stable around the mean, and that the distance from the mean for the number of applications in the latest fiscal year is a large multiple of the standard deviation across the previous years. Currently, the threshold distance from the mean in terms of standard deviations that we are flagging as an issue is `20`. See variable `stdevs_away_from_mean_threshold`.
